...
- 页面配置系统：草稿与发布版本分离，公开页读取发布配置；历史 `links` section 会在读写和渲染时自动清洗；`news` section 的 `cleanPageConfig` 只过滤 `src` 为空的项目，`href` 允许暂时为空（用户可先选图、后填链接）
- 后台编辑骨架：CMS、博客、商店列表 / 详情页已统一为 tab + 单开折叠面板结构；`saveDraft` / `publish` 通过 `configRef` 模式读取最新配置，消除 useCallback 闭包失效导致的编辑内容意外丢失问题；`LoadingState` 不再依赖 `useI18n`，可安全用于 Suspense fallback（I18nProvider 边界以外的场景不再崩溃）
