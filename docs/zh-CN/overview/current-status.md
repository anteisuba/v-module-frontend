...
- `pnpm check`：通过
- `pnpm test`：通过，当前为 `30` 个文件 `102` 个测试
...
- Stripe Connect：已提供卖家收款账户模型、onboarding / dashboard link / account sync API、`/admin/settings/payouts` 页面，以及 destination charge 与 `PLATFORM` fallback 路由；payout settings 页已补充资金流、推荐操作顺序、状态说明和 onboarding 进度提示
...
- Stripe Connect 主链路已可用，但异常状态说明和引导仍可继续增强
- 工程基线已进入“可构建 / 可检查 / 可测试 / 可跑 e2e”的稳定态，且已接入 GitHub Actions + Chromium / Firefox / WebKit 持续执行；当前剩余缺口主要是更细的 flaky 治理和 Stripe Connect 运维闭环打磨
