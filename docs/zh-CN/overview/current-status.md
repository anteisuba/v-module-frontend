...
- 商店系统：后台商品管理、公开商品列表/详情、公开结账接口 `POST /api/shop/checkout`、卖家订单列表、订单状态更新
...
- 后台已有订单页 `app/admin/orders/page.tsx`，但 Dashboard 页面没有入口
- 根路径 `/blog`、`/shop` 仍硬编码重定向到 `/u/xiuruisu/...`，不是通用入口
- `PageConfig` 仍定义 `links` section，但渲染器对该类型直接返回 `null`
