...
- 自动化测试文件：已存在，覆盖认证、页面草稿保存/发布、CMS 浏览器层发布、CMS 发布到 `/u/[slug]` 的公开页回归、媒体库引用跳转替换、blog/news/shop 公开入口与详情页端到端回归、公开页读取、博客/商品/新闻后台编辑接口、Stripe Checkout 创建、Stripe Webhook（含 dispute）、订单详情读取、订单列表筛选/导出、退款路由、支付对账/结算核销接口、内部定时同步接口、邮件通知降级行为、博客评论公开/审核接口
...
- 媒体库：后台已提供统一媒体库页面，支持复用、按使用场景筛选、批量选择、删除保护、引用追踪和跳转到替换入口
...
- 自动化测试目前已覆盖认证、页面草稿保存/发布、CMS 浏览器层发布、CMS 发布到 `/u/[slug]` 的公开页回归、媒体库引用跳转替换、blog/news/shop 公开入口与详情页端到端回归、公开页读取、博客/商品/新闻后台编辑接口、Stripe Checkout / Webhook / dispute、订单详情、订单列表筛选/导出、退款路由、支付对账 / 结算核销接口、内部定时同步接口、邮件通知降级行为和评论审核 API；但尚未扩展到更多后台 UI 渲染链路和更完整的支付浏览器链路
- 当前支付仍只接入 Stripe 单一通道；虽然退款后台、基础对账页、结算核销页、争议处理和自动同步已具备闭环，但 PayPal / 本地支付方式、证据材料提交流程和更深的账单比对仍未完成
- 媒体库已具备删除、批量管理、引用追踪和跳转替换；但更细粒度的资源治理（如引用方直接替换、批量标签维护）仍可继续补强
